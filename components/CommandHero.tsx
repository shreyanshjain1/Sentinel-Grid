...
import { ArrowRight, Crosshair, LockKeyhole, ShieldAlert, Sparkles, Zap } from "lucide-react";
import { tickerItems } from "@/data/security-data";

const heroMetrics = [
  { label: "Cases triaged", value: "42", tone: "text-[#f2b84b]" },
  { label: "Endpoint cover", value: "97%", tone: "text-emerald-200" },
  { label: "Avg response", value: "4m", tone: "text-stone-50" }
];
...
export function CommandHero() {
  return (
    <section className="noise relative min-w-0 overflow-hidden rounded-[1.85rem] border border-stone-100/10 bg-[#17130f] p-4 shadow-[0_40px_110px_rgba(0,0,0,.42)] sm:rounded-[2.25rem] sm:p-6 lg:p-7 2xl:p-8">
      <div className="absolute -left-24 top-16 h-64 w-64 rounded-full bg-[#e24637]/22 blur-3xl" />
      <div className="absolute -right-24 -top-20 h-80 w-80 rounded-full bg-[#f2b84b]/20 blur-3xl" />
      <div className="absolute bottom-0 left-1/3 h-44 w-44 rounded-full bg-emerald-300/10 blur-3xl" />

      <div className="relative z-10 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(380px,.95fr)] xl:items-stretch 2xl:gap-8">
        <div className="flex min-w-0 flex-col justify-between gap-7">
          <div className="min-w-0">
            ...
            <motion.h2
              ...
              className="max-w-5xl break-words text-[clamp(2.55rem,9vw,5.4rem)] font-black uppercase leading-[0.91] tracking-[-0.065em] text-stone-50 xl:text-[clamp(4.2rem,5.8vw,6.25rem)]"
            >
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {heroMetrics.map((metric) => (
              <div key={metric.label} className="rounded-[1.35rem] border border-white/10 bg-black/30 p-4 backdrop-blur-sm transition hover:-translate-y-1 hover:bg-white/[0.055]">
                <p className={`text-2xl font-black tracking-[-0.04em] ${metric.tone}`}>{metric.value}</p>
                <p className="mt-1 text-xs font-bold uppercase tracking-[0.18em] text-stone-500">{metric.label}</p>
              </div>
            ))}
          </div>
        </div>

        <div className="perspective-1000 relative min-h-[340px] min-w-0 overflow-hidden rounded-[1.7rem] border border-stone-100/10 bg-[#0d0d0c]/70 p-4 sm:min-h-[400px] sm:rounded-[2rem] sm:p-5 md:min-h-[440px] md:p-6 xl:min-h-full">
          ...
          <SignalOrb className="float-slow bottom-16 right-14 h-24 w-24 sm:bottom-16 sm:right-20 sm:h-28 sm:w-28" label="Payload contained" />

          <div className="cube absolute left-1/2 top-[46%] h-36 w-36 -translate-x-1/2 -translate-y-1/2 sm:h-44 sm:w-44">
          ...
          <motion.div
            ...
            className="absolute left-1/2 top-[46%] h-60 w-60 -translate-x-1/2 -translate-y-1/2 rounded-full border border-dashed border-[#f2b84b]/35 sm:h-72 sm:w-72"
          />

          <div className="absolute bottom-4 left-4 right-4 rounded-[1.5rem] border border-white/10 bg-black/60 p-4 backdrop-blur-xl sm:bottom-5 sm:left-5 sm:right-5">
